          ...
          {!showLiffQR ? (
            <>
              ...
              <p className="text-xs text-gray-600 mb-3">
                予約確認・リマインドがLINEに届きます。
              </p>
              <button
                type="button"
                onClick={() => setShowLiffQR(true)}
                className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-[#06C755] text-white text-sm font-bold rounded-xl hover:bg-[#05b04c] transition-colors"
              >
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-gray-800 mb-1">スマホのLINEで読み取ってください</p>
              <p className="text-xs text-gray-600 mb-3">
                QRコードを読み取ると、LINEで予約フォームが開きます。
              </p>
              <div className="inline-block bg-white p-3 rounded-xl border border-gray-200">
                <QRCodeSVG value={liffUrl} size={160} />
              </div>
              <button
                type="button"
                onClick={() => setShowLiffQR(false)}
